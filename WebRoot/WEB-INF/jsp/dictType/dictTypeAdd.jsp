...
function save_dictType() {
    var r = $('#dictTypeForm').form('validate');
	if(!r) {
		return false;
	}

	//提交表单 
	$('#dictTypeForm').ajaxSubmit({
		url:'dictTypeController/save.do', //提交给哪个执行
		type:'post',
		dataType:'json',
		success: function(data){
	        if (data && data.result.success == true) {
				api.close();
				W.$('#dictTypeList').datagrid('reload');
			} else {
...
<div region="center" class="detail-view-container">
	<div class="datagrid-toolbar">
		<a href="javascript:void(0);" id="btn-save" class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-save'" onclick="save_dictType();">保存</a>
